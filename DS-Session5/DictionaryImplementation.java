...
public class DictionaryImplementation implements Dictionary {
	...
	public DictionaryImplementation(String initialEntry) {
		...
		try {
			...
			for (Object obj1 : jsonObject.keySet()) {
				addKeyValuePair(obj1.toString(), jsonObject.get(obj1)
						.toString());
			}
		} catch (Exception e) {
	...
	public BSTNode addKeyValuePair(String key, String value) {
		...
		if (isKeyValueValid(key) && isKeyValueValid(value)) {
	...
	public void inOrderTraversal(BSTNode root) {
		if (root != null) {
			...
			sortedList.add(new BSTNode(root.getKey(), root.getValue()));
	...
	public List<BSTNode> sortedListBetweenKeys(String key1, String key2) {
		...
		while (iterator.hasNext()) {
			...
			if (key1.compareToIgnoreCase(entry.getKey()) < 0
					&& key2.compareToIgnoreCase(entry.getKey()) > 0)
				sortedSubList.add(entry);
			else if (key2.compareToIgnoreCase(entry.getKey()) < 0)
				break;
		}
		for (BSTNode node : sortedSubList)
	...
	// return assertion error if key or value is empty
	public boolean isKeyValueValid(String string) {
		if (string.length() == 0)
			throw new AssertionError("Key or Value is empty");
		else
		    return true;
	}
}
