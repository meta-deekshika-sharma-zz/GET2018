...
public class JDBCQuery {
			 ...
			 PreparedStatement statement = conn.prepareStatement(showFriend);)
		{
			...
			if(organization != null)
			{
				...
				while(resultSet.next())
				{
					...
					friendList.add(new UserDAO(firstNameResult, lastNameResult, email, dob, password, contactNumber, organization));
				}
			}
		} catch (SQLException e) {
	...
	{
		String query = "SELECT firstName, lastName, dob, contactNumber, organization FROM User WHERE email = ? AND password = ?";
			...
			PreparedStatement statement = conn.prepareStatement(query);)
		{
			...
			while(resultSet.next())
			{
				...
				return new UserDAO(firstNameResult, lastNameResult, email, dob, password, contactNumber, organization);
			}
	...
	public static String getOrganizationName(String email)
	{
		...
		try(Connection conn = JDBCConnection.getConnection();
			PreparedStatement statement = conn.prepareStatement(organization);)
			{
				statement.setString(1, email);
