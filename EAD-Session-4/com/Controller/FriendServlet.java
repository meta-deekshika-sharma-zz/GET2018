...
public class FriendServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<!DOCTYPE html>" +
		          "<html>" + "<head>" + "<title>Friend List</title>" + 
		          cssFile + "</head>" +
		          "<body>" + "<h1 align = 'center'>Friends</h1>" + "<form>" + "<table id = 'updateTab'>" + "<tr>" +
		...
		for(UserDAO friend: friendList)
		{
			...
			out.print("<tr>" + "<td id = 'row'>" + friend.getFirstName() + "</td>");
			out.print("<td id = 'row'>" + friend.getLastName() + "</td>");
			out.print("<td id = 'row'>" + friend.getEmail() + "</td>");
			out.print("<td id = 'row'>" + "<a href='FriendDetailServlet?email=" + friend.getEmail() + "&firstName=" + friend.getFirstName() + "&lastName=" + friend.getLastName() +
					"&dob=" + friend.getDob() + "&contactNumber= " + friend.getContactNumber() +"'>Open</a>" +"</td>" +  "</tr>");
		}
