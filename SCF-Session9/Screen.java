...
/**
 * This class will have a virtual screen and have method to display shapes on
 * screen
 * 
...
public class Screen {
	...
	public boolean addShape(Shape shapeObject) {
		if (shapeObject != null && shapeObject.getOrigin().getX() <= xMax
				&& shapeObject.getOrigin().getX() >= 0
				&& shapeObject.getOrigin().getY() <= yMax
				&& shapeObject.getOrigin().getY() >= 0) {
			screenObject.put(shapeObject, new Date());
			return true;
		} else
			throw new AssertionError(
					"The given shape object is not added into screen!");
	}
	...
	/**
	 * This method will find list of shapes which are enclosed to a specified
	 * point
	 * 
	 * @param point
	...
	/**
	 * This method will sort the shapes on screen according to area, parameter,
	 * origin distance or by time stamp
	 * 
	 * @param sortedType
	 * @return list of shapes in sorted way
	 */
	public List<Shape> sortedShapeObject(ShapeSorted sortedType) {
		List<Shape> listOfSortedShape = new ArrayList<Shape>(
				screenObject.keySet());

		switch (sortedType) {
		case AREA:
			Collections.sort(listOfSortedShape,
					(o1, o2) -> (int) (o1.getArea() - o2.getArea()));
			break;
		case ORIGINDISTANCE:
			Collections.sort(listOfSortedShape, (o1, o2) -> (int) (o1
					.getOriginDistance() - o2.getOriginDistance()));
			break;
		case PERIMETER:
			Collections.sort(listOfSortedShape,
					(o1, o2) -> (int) (o1.getPerimeter() - o2.getPerimeter()));
			break;
		case TIMESTAMPS:
			Collections.sort(listOfSortedShape, (o1, o2) -> (o1
					.getCurrentDate().compareTo(o2.getCurrentDate())));
			break;
	...
	public void showList(List<Shape> listOfShape) {
		for (Shape shape : listOfShape) {
			System.out
					.println(shape.getArea() + " " + shape.getPerimeter() + " "
							+ shape.getOriginDistance() + " "
							+ shape.getOrigin() + " " + shape.getShapeType()
							+ " " + shape.getCurrentDate());
		}
	}
}
