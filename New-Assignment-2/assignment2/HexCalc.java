...
public class HexCalc implements HexInterface {
	...
	/**
	 * this method will check if input hexadecimal no. is valid or not
	 * @param hexaNumber take hexadecimal no. as parameter
	 * @return true if no. is valid otherwise false
	 */
	public boolean isNumberValid (String hexaNumber)
	{
		for (int i = 0; i < hexaNumber.length(); i++)
		{
			if (hexaNumber.charAt(i) >= 70)
			{
				return true;
			}
		}
	...
	{
		int add = decimalValue(hexaFirst) + decimalValue(hexaSecond);
		return hexadecimalValue(add);
	}
	...
	{
		int sub = decimalValue(hexaFirst) - decimalValue(hexaSecond);
		return hexadecimalValue(sub);
	}
	...
	{
		int mul = decimalValue(hexaFirst) * decimalValue(hexaSecond);
		return hexadecimalValue(mul);
	}
	...
	{
		int div = decimalValue(hexaFirst) + decimalValue(hexaSecond);
		return hexadecimalValue(div);
	}
